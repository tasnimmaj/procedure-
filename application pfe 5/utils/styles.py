...
def get_logo_base64():
    """
    Convertit le logo en base64 pour l'intégrer directement dans le CSS.
    Cela évite les problèmes de chemin et assure que le logo s'affiche partout.
    """
    try:
        logo_path = Path("C:/Users/MAJOUL/Desktop/application pfe/application pfe 4/logozitouna.png")
        if logo_path.exists():
            with open(logo_path, "rb") as f:
                data = f.read()
                return base64.b64encode(data).decode()
        else:
            # Si le logo n'est pas trouvé dans le répertoire courant, essayer d'autres emplacements
            possible_paths = [
                Path() / "logozitouna.png",
                Path("assets") / "logozitouna.png",
                Path("images") / "logozitouna.png",
                Path("static") / "logozitouna.png"
            ]
...
def apply_green_theme():
    """
    Applique le thème vert complet à la page Streamlit avec le logo Zitouna.
    À appeler au début de chaque page pour maintenir la cohérence visuelle.
    """
    logo_base64 = get_logo_base64()
    
    # CSS avec ou sans logo selon sa disponibilité
    logo_css = ""
    if logo_base64:
        logo_css = f'<img src="data:image/png;base64,{logo_base64}" alt="Logo Zitouna" style="width: 100%; height: 100%; object-fit: contain; border-radius: 50%;">'
    else:
        logo_css = '<div style="color: white; font-size: 24px; font-weight: bold;">🏦</div>'
    ...
    # Affichage du header avec le logo
    st.markdown(f"""
        <div class="app-header">
            <div>
                <h1 style="margin: 0; color: white; font-size: 1.8rem;">Gestionnaire de Procédures Internes</h1>
                <p style="margin: 0; color: rgba(255,255,255,0.9); font-size: 1rem;">Banque Zitouna - Transformation des Notes Circulaires</p>
            </div>
