...
def main():
    ...
    # Récupérer le tableau source et le tableau I/O
    if 'procedure_generee' in st.session_state:
        procedure_data = st.session_state.get('procedure_generee', {})
        markdown_text = procedure_data.get('procedure', '') if isinstance(procedure_data, dict) else procedure_data
        io_table = procedure_data.get('io_table', '') if isinstance(procedure_data, dict) else ''
    ...
    try:
        if markdown_text:
            ...
            # Générer le diagramme avec le tableau I/O
            flowchart_base64, error = generate_flowchart(markdown_text, procedure_name, io_table)
    ...
    with st.expander("ℹ️ Informations sur les logigrammes"):
        ...
        * Début et fin représentent les entrées et sorties du processus
