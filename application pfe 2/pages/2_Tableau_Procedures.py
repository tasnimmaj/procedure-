...
def extract_procedure_components(procedure_text):
    ...
    i = 0
      # Définir directement le tableau I/O simplifié avec entrée initiale et sortie finale
    components["io"] = """| Evènement | Processus en interface | Description du processus en interface |
| --- | --- | --- |
...
| Sortie | Application de la note | Mise en place effective des directives de la note circulaire |"""
    ...
    # Continuer avec le reste de l'extraction comme avant
    while i < len(lines):
        line = lines[i].strip()
    ...
    if current_section_type and current_section:
        components[current_section_type] = "\n".join(current_section)
      # S'assurer que le tableau des étapes a un format cohérent avec une ligne de séparation
    if components["etapes"]:
        ...
        if header_line_index >= 0:
            ...
            # Supprimer toute ligne de séparation existante après l'en-tête
            next_line_idx = header_line_index + 1
            if next_line_idx < len(lines) and "---" in lines[next_line_idx]:
                lines.pop(next_line_idx)
            
            # Insérer la nouvelle ligne de séparation
            lines.insert(header_line_index + 1, separator_line)
            
            # Reconstruire le tableau
            components["etapes"] = "\n".join(lines)
    
    # Une fois le tableau des étapes extrait, le sauvegarder dans la session
    if components["etapes"]:
        st.session_state.generated_markdown_table = components["etapes"]
        st.session_state.has_generated_table = True  # Indicateur que le tableau a été généré
    
    return components

# Après la génération du tableau dans la page 2
def generate_markdown_table(steps):
    markdown_table = "| # | Activité | Acteur |\n|---|----------|--------|\n"
    for i, step in enumerate(steps, 1):
        markdown_table += f"| {i} | {step['activity']} | {step['actor']} |\n"
    
    # Sauvegarder dans la session pour la page 3
    st.session_state.generated_markdown_table = markdown_table
...
def main():
    ...
    if st.session_state.get("procedure_generee"):
        ...
        if procedure_components["etapes"]:
          ...
          # Afficher le tableau I/O simplifié
        ...
        if procedure_components["scenarios_ko"] and procedure_components["scenarios_ko"].strip():
            ...
            1. **Non-validation de la procédure**
               ...
               - Non-respect des règles d'archivage""")        # Options de navigation et téléchargement
